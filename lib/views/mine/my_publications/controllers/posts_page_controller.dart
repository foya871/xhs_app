...
class PostsPageController extends BaseRefreshController
  ...
  RxList<PublicationsPostsModel> publications = <PublicationsPostsModel>[].obs;
  RxList<PublicationsPostsModel> underReviews = <PublicationsPostsModel>[].obs;
  RxList<PublicationsPostsModel> rejecteds = <PublicationsPostsModel>[].obs;
  ...
  void initHttpDatas() {
    for (int i = 0; i < tabs.length; i++) {
      getPublicationsPostsList(isRefresh: true, currentTabIndex: i);
    }
  }
  ...
  Future<IndicatorResult> onRefresh() async {
    ...
    return await getPublicationsPostsList(
  ...
  Future<IndicatorResult> getPublicationsPostsList({
    ...
    try {
      ...
      final response = await Api.getPublicationsPostsList(
        userId: service.user.userId ?? 0,
        status: _getVideoStatus(status),
  ...
  void _updatePublicationsList(VideoStatus status,
      List<PublicationsPostsModel> response, bool isRefresh) {
    switch (status) {
      case VideoStatus.published:
        if (isRefresh) {

 ...
 * @LastEditTime: 2024-10-16 21:08:36
 * @FilePath: /xhs_app/lib/src/views/mine/favorite/views/favorite_video_page.dart
...
class FavoriteVideoPage extends StatefulWidget {
  final bool isBuy;
  const FavoriteVideoPage({super.key, this.isBuy = true});

  @override
  State<FavoriteVideoPage> createState() => _StarListData();
}

class _StarListData extends State<FavoriteVideoPage> {
  final _pageSize = 20;
  ...
  Future _initListData(int pageKey) async {
    final videos = await httpInstance.get<VideoBaseModel>(
        url: widget.isBuy ? 'video/userPurVideo' : 'video/userFavorites',
        queryMap: {
          'pageSize': _pageSize,
          'videoMark': 1,
          'page': pageKey,

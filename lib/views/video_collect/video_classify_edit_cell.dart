...
// 编辑video
class VideoClassifyEditCell extends StatefulWidget {
  final List<VideoClassifyModel> selected;
  final List<VideoClassifyModel> optional;
  final Function(VideoClassifyModel)? onTapChangeClassify;
  final Function(
    List<VideoClassifyModel> selected,
    List<VideoClassifyModel> optional,
  )? onEditDone;

  VideoClassifyEditCell({
    super.key,
    required List<VideoClassifyModel> selected,
    required List<VideoClassifyModel> optional,
...
class _State extends SafeState<VideoClassifyEditCell> {
  bool _editing = false;
  ...
  bool _isReorderableModel(VideoClassifyModel model) =>
      widget.selected.contains(model);

  bool _inSelected(VideoClassifyModel model) =>
      widget.selected
  ...
  Widget _buildOneSelected(VideoClassifyModel model, int i) => Container(
  ...
  Widget _buildSelectedClassify() => AnimatedReorderableGridView(
        ...
        onReorder: (oldIndex, newIndex) {
          if (!_isReorderableModel(widget.selected[newIndex])) {
            return;
          }
          setState(() {
  ...
  Widget _buildOneOptional(VideoClassifyModel model) {
    final inSelected = _inSelected(model);
    // logger.d('_buildOneOptional ${widget.selected.length}');
    return Stack(
      children: [
        Container(
          width: 74.w,

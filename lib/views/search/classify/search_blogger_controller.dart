...
class SearchBloggerController extends BaseRefreshController
    with BaseRefreshPageCounter {
  final PagingController<int, BloggerList> pagingControllers =
  ...
  Future<IndicatorResult> onRefresh() async {
    resetPage();
    return await getBloggerList(isRefresh: true);
  }

  @override
  Future<IndicatorResult> onLoad() async {
    incPage();
    return await getBloggerList();
  }

  Future<IndicatorResult> getBloggerList({bool isRefresh = false}) async {
    try {
      if (isRefresh) {
      ...
      if (GetUtils.isBlank(response) == false) {
        ...
        pagingControllers.appendPage(response?.bloggerList ?? [], page);

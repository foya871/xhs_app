...
class SearchPhotoController extends BaseRefreshController
    with BaseRefreshPageCounter {
  final PagingController<int, PictureList> pagingControllers =
  ...
  Future<IndicatorResult> onRefresh() async {
    resetPage();
    return await getPhotoList(isRefresh: true);
  }
  ...
  Future<IndicatorResult> getPhotoList({bool isRefresh = false}) async {
    try {
      if (isRefresh) {
      ...
      if (GetUtils.isBlank(response) == false) {
        ...
        pagingControllers.appendPage(response?.pictureList ?? [], page);
